		    ...
		    input :
				  ...
				  in builtins.foldl' reducers.processed initial indices ;
					...
					track = caller index path element ;
				        in builtins.trace "YES ${ if is-simple then "SIMPLE" else if is-list then "LIST" else "SET ${ builtins.toString previous } ${ builtins.toString track.size }" }" ( previous + track.size ) ;
                                } ;
